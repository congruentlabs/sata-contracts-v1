...
contract Swap is ISwap {
  ...
  bytes constant internal DOMAIN_NAME = "VERISWAP";
  bytes constant internal DOMAIN_VERSION = "1";

  // Unique domain identifier for use in signatures (EIP-712)
  bytes32 private _domainSeparator;

  // Possible nonce statuses
  bytes1 constant internal AVAILABLE = 0x00;
  bytes1 constant internal UNAVAILABLE = 0x01;
  ...
  // Reference to the SignataIdentity contract
  ...
  /**
    ...
    */
  constructor(TransferHandlerRegistry swapRegistry, address _signataIdentity) public {
  ...
  function swap(Types.Order calldata order) external {
    ...
    require(signerNonceStatus[order.signer.wallet][order.nonce] == AVAILABLE, "ORDER_TAKEN_OR_CANCELLED");

    // Ensure the order nonce is above the minimum.
    require(order.nonce >= signerMinimumNonce[order.signer.wallet], "NONCE_TOO_LOW");
    ...
    if (order.sender.wallet == address(0)) {
      ...
      /**
        ...
        */
      require(isSenderAuthorized(order.sender.wallet, msg.sender), "SENDER_UNAUTHORIZED");
    ...
    if (order.signature.v == 0) {
      /**
        ...
        */
      require(isSignerAuthorized(order.signer.wallet, msg.sender), "SIGNER_UNAUTHORIZED");
    } else {
      /**
        * The signature is provided. Determine whether the signer is
        * authorized and if so validate the signature itself.
        */
      require(isSignerAuthorized(order.signer.wallet, order.signature.signatory), "SIGNER_UNAUTHORIZED");

      // Ensure the signature is valid.
      require(isValid(order, _domainSeparator), "SIGNATURE_INVALID");
    }

    if (order.enforceIdentity == true) {
  ...
  /**
    ...
    */
  function isValid(Types.Order memory order, bytes32 domainSeparator) internal pure returns (bool) {
